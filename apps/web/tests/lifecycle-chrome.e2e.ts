...
describe('web e2e: lifecycle & chrome (workspace flow / reload / dark mode)', () => {
  ...
  it.skipIf(MODE === 'record')('keeps the command menu below the visible viewport edge after keyboard panning', async () => {
    const mobilePage = await newEnglishPage(browser)
    const mobileTripwire = watchConsole(mobilePage)
    onTestFailed(() => saveFailureShot(mobilePage, 'web-e2e-command-menu-viewport'))
    try {
      await mobilePage.setViewportSize({ width: 390, height: 844 })
      await mobilePage.goto(scaffold.authenticatedUrl, { waitUntil: 'load' })
      await mobilePage.getByRole('button', { name: 'Add files or run commands' }).click()
      const menu = mobilePage.locator('[data-trigger-menu]')
      await menu.getByRole('option').first().waitFor({ timeout: 10_000 })
      await menu.getByRole('status').waitFor({ state: 'hidden', timeout: 10_000 })
      const before = await menu.boundingBox()
      expect(before).not.toBeNull()
      expect(before!.height).toBeGreaterThan(160)
      const bottom = before!.y + before!.height
      const visibleTop = bottom - 84 - 160
      expect(visibleTop).toBeGreaterThan(0)
      await mobilePage.evaluate((offsetTop) => {
        const viewport = window.visualViewport!
        Object.defineProperty(viewport, 'offsetTop', { configurable: true, value: offsetTop })
        viewport.dispatchEvent(new Event('scroll'))
      }, visibleTop)
      await expect.poll(async () => (await menu.boundingBox())?.height).toBeCloseTo(160, 0)
      const after = await menu.boundingBox()
      expect(after!.y).toBeGreaterThanOrEqual(visibleTop + 84 - 1)
      expect(after!.y + after!.height).toBeCloseTo(bottom, 0)
      await mobilePage.locator('[data-composer-input]').first().press('Escape')
