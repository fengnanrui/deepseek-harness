...
使用 `useAnchoredMaxHeight` 的底部锚定命令与选择菜单，会在键盘平移或缩放后限制高度，避免越过可视视口顶部。该钩子保留调用方的锚点、设计高度上限以及窗口框架或调用方指定的顶部安全间距，并在浮层关闭时释放窗口与可视视口监听器。

使用 `useAnchoredPosition` 的浮动面板会在软键盘或缩放改变可视视口时跟随更新。面板大于视口时仍保持起始边缘可见；可滚动尺寸仍由调用方负责。标准 `Modal` 的内容在动态视口内滚动，底部操作保持可达。复制反馈会抑制重复的未完成写入，在文本变化时重置，并在卸载时释放定时器。
