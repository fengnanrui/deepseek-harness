...
Bottom-anchored command and selection menus using `useAnchoredMaxHeight` fit below the visual viewport's top edge after keyboard panning or zoom. The hook keeps the caller's anchor, design height cap, and frame or caller-supplied top clearance, and releases its window and visual-viewport listeners when the overlay closes.

Floating panels using `useAnchoredPosition` follow the visual viewport when a software keyboard or zoom changes it. A panel larger than that viewport keeps its leading edges visible; the caller still owns its scrollable size. Standard `Modal` content scrolls within the dynamic viewport while footer actions remain reachable. Copy feedback suppresses duplicate pending writes, resets when its text changes, and releases its timer on unmount.
